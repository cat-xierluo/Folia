...
- **修复编辑器选中文字后按 Backspace/Delete 偶发未生效的问题**（ISS-69）：在 WYSIWYG（Vditor IR）模式下，删除选中文本后界面短暂闪烁、文字恢复、光标回到原位置。根因是 `sanitizeIrDom` 的触发条件仅看 `changed`（任何字节级差异），DOMPurify / 浏览器 HTML parser 的序列化规范化（属性顺序、SVG 属性大小写、自闭合标签、空白等）会让 IR DOM 被整体重写，摧毁 Selection 并经 `onChange → 父 source → useEffect → setValue` 反馈环路恢复原文。修复后：1）`sanitizeVditorIrHtml` 新增 `securityChanged` 字段，仅在确实剥除危险节点/属性/URI 时才整体重写 IR DOM；2）`sanitizeIrDom` 在整体重写前后用文本偏移快照保留 Selection；3）`input()` 回调仅在 `securityChanged` 时才用 `editor.getValue()` 覆盖 callback 参数，避免父组件反馈。新增 4 项 service 单测与 3 项组件烟雾测试覆盖纯文本 / 仅序列化 / 真剥除三类场景。
