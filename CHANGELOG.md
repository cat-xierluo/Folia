...
- **修复固定大纲与 Word/HTML 导出栏同开时的窄列排版（ISS-223）**：修正未命中的主区宽度选择器，三栏按实际窗口分配宽度。默认 980px 窗口仍保留固定大纲和右栏，必要时缩窄大纲显示宽度与 Markdown 正文两侧留白，并清除编辑器内层未随侧栏缩小的重复留白；Word 纸张按右栏实际宽度缩放，避免被裁切。窗口变宽或关闭右栏后，大纲恢复用户设置的宽度。
...
- **固定大纲宽度可调节（ISS-220，PR #171 承接 @Yillan-lamb 的原始方案 #166）**：固定态大纲左栏右缘新增拖拽手柄（样式与右侧 Word 预览 resizer 同构），可拖动调整大纲宽度（200px ~ 容器 40%，上限 480px），双击恢复默认 260px。两个 resizer 的 `dragging` 高亮相互独立（共用 `is-resizing` 仅控制拖拽期间的文本选中抑制）。未固定时轻量横线轨道与展开面板行为不变。
