...
- **固定大纲宽度可调节（ISS-217）**：固定态大纲左栏右缘新增拖拽手柄（样式与右侧 Word 预览 resizer 同构），可拖动调整大纲宽度（200px ~ 容器 40%，上限 480px），双击恢复默认 260px。两个 resizer 的 `dragging` 高亮相互独立（共用 `is-resizing` 仅控制拖拽期间的文本选中抑制）。未固定时轻量横线轨道与展开面板行为不变。

- **「总是固定大纲」开关收敛到设置页（ISS-217，触碰 DESIGN.md 记载的面板内开关设计）**：原先固定态大纲面板头部下方的「总是固定大纲」开关按钮移除，改在「设置 → 外观」分区提供同名开关（新增说明文案，三语言同步）。AppLayout 经 `useSettings` 订阅变更即时生效：开启后当前及后续含标题文档默认固定大纲；在设置页关闭后，当前文档大纲随之取消固定——**无论固定来自该偏好还是手工固定**（关闭全局开关会同步清除会话固定）。大纲面板内的固定 / 取消固定按钮保留——固定态下点击「取消固定大纲」仍会顺带关闭「总是固定大纲」偏好（否则设置项会把大纲立即钉回）。
