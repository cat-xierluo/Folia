...
### [DEC-145] - 2026-09-06 - 固定大纲宽度可调节 + 「总是固定大纲」双入口（ISS-220，PR #166 由 maintainer 承接整合；原登记 ISS-217/DEC-143 撞号改号）

**背景**：用户反馈两点：①固定态大纲左栏宽度固定 260px 不可调，长标题条目换行局促；②「总是固定大纲」开关只在大纲面板内，希望有设置页入口。原 PR（#166）把面板内开关整体移入设置页并附带「设置页关闭→强制解除一切固定」语义；maintainer review 后拍板调整为**双入口**并修正关闭语义，由 maintainer 承接整合（fork 分支冲突 + 改号 ISS-220/DEC-145 + 拆出版本号切版提交，0.8.1 定版由维护者统一执行）。

**决策**：

1. **宽度调节走既有 resizer 模式，不新增设置项**：固定态大纲右缘加拖拽手柄（`.toc-resizer`，样式与 `.word-preview-resizer` 同构），宽度经 `--toc-width` CSS 变量驱动 `.floating-toc.pinned`，上下限 200px ~ `min(480px, 容器 40%)`，双击恢复 260px；宽度为会话内状态不持久化（与 `rightPanelWidth` 一致）。两个 resizer 的拖拽态用 `resizingPanel: 'toc' | 'right' | null` 判别联合，`dragging` 高亮各手柄独立判定。
2. **「总是固定大纲」双入口**：「设置 → 外观」新增同名 toggle（三语言 label/desc），面板内 `floating-toc-preference` 按钮**保留**——两处入口读写同一 `tocAlwaysPinned`，`settingsService` schema 零改动。撤回原 PR 的「面板内开关移除 + FloatingToc 组件变纯」：面板入口是就地可见即改的高频入口，收走后改默认要绕道设置页二级分区。
3. **关闭语义：偏好驱动固定随之解除，手工固定不受影响**（否决原 PR 的「设置页关闭→同步清除会话固定」effect）。理由：a) 同一开关两处入口若行为相反（面板关=当前文档转会话固定保留视图，设置关=连手工固定一起清），用户无法建立稳定预期，且 effect 会在面板入口关闭路径上把 `setTocSessionPinned(true)` 反向清掉（事件时序冲突，实测语义破坏）；b) 改「默认固定」偏好不应动用户显式建立的会话固定——解除手工固定走面板「取消固定大纲」按钮（该按钮顺带关闭偏好是既有必要联动，否则偏好会立即钉回）。开启方向由 `tocPinned = tocSessionPinned || tocAlwaysPinned` 合成自然完成。

**影响**：DESIGN.md Floating TOC 段改写为双入口口径；e2e 调整三个用例（面板内按钮断言恢复、设置页开关流程保留并覆盖拖拽/双击复位、新增「设置页关闭保留手工固定」语义锁定用例）；未新增依赖。WKWebView 真机拖拽手感 NOT_VERIFIED（移交）。
