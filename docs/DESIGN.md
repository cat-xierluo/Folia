...
TOC 不再作为顶部按钮出现。文档存在标题时，内容区左侧默认显示一列极弱的浮动横线刻度，表示大纲存在但不抢占阅读空间；起点控制在主内容区顶部下方约 48px，跟随更紧凑的 Markdown 显示面。鼠标移入、点击或键盘聚焦横线轨道时向右展开半透明标题列表；横线轨道只负责轻量查看大纲，不直接切换固定状态。展开面板右上角提供明确的固定按钮和关闭按钮：固定按钮把大纲变为左侧常驻栏并让正文区域让位，关闭按钮收起面板并回到轻量横线入口。固定状态下按钮切换为取消固定，关闭按钮则直接收回到轻量横线状态。“总是固定大纲”作为持久开关控制后续含标题文档是否默认固定，为双入口：固定态大纲面板头部内保留该开关，「设置 → 外观」分区提供同名开关，两处读写同一偏好。关闭开关只解除偏好驱动的固定，手工固定的当前文档不受影响（解除手工固定走「取消固定大纲」按钮，该按钮顺带关闭该偏好，否则偏好会把大纲立即钉回）；面板内开关关闭时当前文档转为会话固定、视图保持不变。固定态大纲左栏右缘提供拖拽手柄（与 Word 预览 resizer 同构），宽度可在 200px ~ 容器 40% 间调整，双击恢复默认 260px。未固定时，从横线轨道移动到展开面板不能出现 hover 断层，面板条目必须可点击。长标题文档下，折叠轨道和展开列表都必须限制在状态栏上方并允许内部滚动。
...
TOC 只收录 Markdown 正文中的 ATX 标题；代码围栏内的 `#` 示例、HTML block / 图表预览产生的标题元素均不进入导航序列。源码模式与 WYSIWYG 必须共用同一标题模型，重复标题也要按源码顺序绑定独立稳定锚点。点击导航后的定位应快速完成：近距离可用短平滑保留方向感，超过约 1.5 个编辑器视口时即时跳转，避免长文滚动动画持续数秒；系统开启“减少动态效果”时统一即时定位。
...
固定大纲与 Word/HTML 导出栏同时打开时，三栏始终保持并排。默认 980px 窗口下，左侧大纲可临时缩至 200px，右栏不低于 360px，中间 Markdown 区优先保持约 400px；正文两侧内边距随中间栏收紧，真实段落宽度至少 360px。导出栏打开时，正文留白由编辑器外层统一控制，避免 Vditor 内层固定像素留白重复占位。关闭导出栏或窗口加宽后，大纲恢复用户设置的宽度，不修改固定偏好。Word 纸张缩放应依据右栏的实际布局宽度。
