...
TOC 不再作为顶部按钮出现。文档存在标题时，内容区左侧默认显示一列极弱的浮动横线刻度，表示大纲存在但不抢占阅读空间；起点控制在主内容区顶部下方约 48px，跟随更紧凑的 Markdown 显示面。鼠标移入、点击或键盘聚焦横线轨道时向右展开半透明标题列表；横线轨道只负责轻量查看大纲，不直接切换固定状态。展开面板右上角提供明确的固定按钮和关闭按钮：固定按钮把大纲变为左侧常驻栏并让正文区域让位，关闭按钮收起面板并回到轻量横线入口。固定状态下按钮切换为取消固定，关闭按钮则直接收回到轻量横线状态。“总是固定大纲”作为持久开关控制后续含标题文档是否默认固定，入口收敛在「设置 → 外观」，不在大纲面板内出现（面板内只保留固定 / 取消固定与关闭按钮）；在设置页关闭后当前文档大纲随之取消固定，固定态下点击「取消固定大纲」会顺带关闭该偏好。固定态大纲左栏右缘提供拖拽手柄（与 Word 预览 resizer 同构），宽度可在 200px ~ 容器 40% 间调整，双击恢复默认 260px。未固定时，从横线轨道移动到展开面板不能出现 hover 断层，面板条目必须可点击。长标题文档下，折叠轨道和展开列表都必须限制在状态栏上方并允许内部滚动。
