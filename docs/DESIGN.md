...
| Toolbar 图标按钮 | 26px | — | lucide icon | 打开、保存、另存、导出、大纲、设置 |
...
- 高度 36px，**透明背景**（融入内容区）
- 左侧：wordmark + 细分隔线 + icon-only 文件操作（打开、保存、另存、导出 Word）+ 文件名（文件名默认 `opacity: 0`，打开文件后渐入）
- 右侧：icon-only 大纲开关 + Settings 按钮
- `user-select: none`
