...
- **2026-08-16** ISS-194 PR #131 独立 review 回合（Explore reviewer verdict: REQUEST_CHANGES）：**M-1 已修**——CRLF 文档 `split('\n')` 残留 `\r` 让 `FENCE_CLOSE_PATTERN` 永不命中，首个围栏后整篇被当代码块、图片全部不归一化（node 实测复现）；修复为剥离 `\r` 参与判定、输出原样回接（换行风格逐字节保留），+4 项 CRLF 守卫测试，单文件 25/25、全量 749/749、lint/build 0 error。**M-2 核实为不复现**（reviewer 的 title 转义场景实测输出正确，疑似其脚本 shell 转义伪影），未改。**m-1（HTML 属性内字面文本被归一化）登记进 DEC-140 已知边界**；m-2~m-5、n-1~n-7 为次要 / 已登记边界 / 验证性确认，不改。issue #130 / PR #131。
