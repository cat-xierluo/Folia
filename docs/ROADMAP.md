...
- **2026-07-18**
  ...
  - DEC-119 / ISS-179 §9.2 渲染保真度断言 + foreignObject 修复（commit `ca10dcd`）。(1) 修复 `src/services/sanitizeService.ts`：DOMPurify svg profile 默认剥 `<foreignObject>`，使 Mermaid flowchart（默认 `htmlLabels:true`，节点文字在 foreignObject 内）在 HTML / Word 预览出现「有框无字」伪渲染；`ADD_TAGS:['foreignObject']` 显式保留，内部 HTML 仍按 html profile 清洗（script/on* 剥离），安全不降级。(2) 新增 `e2e/mermaid-fidelity.spec.ts` 3 用例：节点文字「开始」「结束」可见、mermaid svg `getBBox` 像素非空、`onload=`/`javascript:` 属性在主 IR + HTML 预览均剥离（这正是 §9.2 完成标准「不再仅用存在 `<svg>` 验收」的落实点）。(3) 新增 `src/services/sanitizeService.test.ts` 2 用例。(4) `.github/workflows/ci.yml` playwright job 追加该 spec。ISS-179 §九.2（节点文字 / 像素非空 / 错误状态 / 最终 sanitize 均有断言）达成。
- **2026-07-16**
