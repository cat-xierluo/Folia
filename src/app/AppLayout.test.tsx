...
describe('AppLayout ISS-200 IO 错误用户反馈', () => {
  let host: HTMLDivElement;
  ...
  beforeEach(() => {
    host = document.createElement('div');
    document.body.append(host);
    fileServiceMock.openPath.mockReset();
    fileServiceMock.saveFile.mockReset();
  });
  ...
  // 触发链:handleOpenPath 由桌面打开/系统打开/欢迎页近期文件等调用,均需
  // Tauri 事件通道;jsdom 下走非 Tauri 分支验证「不 unhandled rejection +
  // console.error 留痕」契约(Tauri message 弹窗分支与 fileService 既有
  // notifyOversized 同构,由 typecheck/i18n 键测试保证)。
  it('openPath 失败被 catch:console.error 留痕且无 unhandled rejection', async () => {
    const errSpy = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    fileServiceMock.openPath.mockRejectedValue(new Error('No such file or directory'));
    ...
    // AppLayout 正常渲染(未因异步拒绝崩树)
    expect(host.querySelector('.app-layout')).not.toBeNull();
    // 无 unhandled rejection:进程内未捕获错误为空(jsdom unhandled 检测由
    // setupVitest 兜底,此处以组件存活 + spy 不抛为代理断言)
    errSpy.mockRestore();
  });
});
